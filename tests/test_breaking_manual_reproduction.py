...
class ManualBreakingReproductionTests(unittest.TestCase):
    ...
    @mock.patch.dict(os.environ, {
        "CONFIRMED_BREAKING_EVENT": EVENT,
        "CONFIRMED_BREAKING_OCCURRED_AT": TODAY_OCCURRED_AT,
    }, clear=False)
    @mock.patch.object(entry.breaking_watch, "watch")
    @mock.patch.object(entry.subprocess, "call", return_value=0)
    def test_confirmed_event_bypasses_watcher_and_forces_dry_run(self, call, watch):
        rc = entry.run()
    ...
    def test_strict_runner_forces_review_only_even_if_caller_requests_post(self, call):
        entry._run_strict_news_bot({
            "PINNED_EVENT": EVENT,
            "PINNED_EVENT_OCCURRED_AT": TODAY_OCCURRED_AT,
            "POST_TO_SNAPCHAT": "1",
        })
        env = call.call_args.kwargs["env"]
        self.assertEqual(env["POST_TO_SNAPCHAT"], "0")
    ...
    @mock.patch.dict(os.environ, {
        "CONFIRMED_BREAKING_EVENT": EVENT,
        "CONFIRMED_BREAKING_OCCURRED_AT": TODAY_OCCURRED_AT,
        "BREAKING_RUN_MODE": "repair_visual",
    }, clear=False)
    @mock.patch.object(entry.breaking_watch, "watch")
    @mock.patch.object(entry.subprocess, "call", return_value=0)
    ...
    @mock.patch.dict(os.environ, {
        "CONFIRMED_BREAKING_EVENT": EVENT,
        "CONFIRMED_BREAKING_OCCURRED_AT": TEST_OCCURRED_AT,
    }, clear=False)
    ...
    @mock.patch.object(entry.breaking_watch, "PERSIST_REVIEW_STATE", True, create=True)
