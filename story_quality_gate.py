#!/usr/bin/env python3
"""Global, deterministic pre-review quality gate for six-card Stories.

This module has no renderer, network, model, Telegram, or Snapchat side effects.
It judges the final editorial frame payloads together with persisted visual
evidence and returns structured BLOCK/WARN findings plus frame-scoped repairs.
"""
...
_CURRENT_CUES = (
    "اليوم", "حالياً", "حاليا", "الآن", "الان", "في الوقت الحالي", "حال الوقت",
...
_EXCLUSIVITY_PATTERNS = (
    re.compile(r"\bالوحيد(?:ة)?\b"),
    re.compile(r"\bحصراً\b"),
    re.compile(r"\bحصرا\b"),
    re.compile(r"\bلا\s+يوجد\s+غير\b"),
    re.compile(r"\bالطريق\s+الوحيد\b"),
    re.compile(r"\bonly\b", re.IGNORECASE),
    re.compile(r"\bsole\b", re.IGNORECASE),
    re.compile(r"\bexclusively\b", re.IGNORECASE),
)

_UNIVERSALITY_PATTERNS = (
    re.compile(r"\bدائماً\b"),
    re.compile(r"\bدائما\b"),
    re.compile(r"\bالجميع\b"),
    re.compile(r"\bكل\s+الناس\b"),
    re.compile(r"\bأبداً\b"),
    re.compile(r"\babsolutely everyone\b", re.IGNORECASE),
    re.compile(r"\balways\b", re.IGNORECASE),
    re.compile(r"\bnever\b", re.IGNORECASE),
)

_SUPERLATIVE_PATTERNS = (
    re.compile(r"\bالأول\s+في\s+العالم\b"),
    re.compile(r"\bالأكبر\s+في\s+العالم\b"),
    re.compile(r"\bالأغلى\s+في\s+العالم\b"),
    re.compile(r"\bthe\s+(?:first|largest|biggest|most expensive)\s+in\s+the\s+world\b", re.IGNORECASE),
)

_SAUDI_CONTEXT_TERMS = (
    "السعود", "المملكة العربية السعودية", "المملكه العربيه السعوديه", "الرياض",
...
def _frame_blob(frame: dict[str, Any]) -> str:
    ...
    for key in ("heading", "text", "punch"):
        parts.append(_clean(frame.get(key)))
...
def _copy_blob(frame: dict[str, Any]) -> str:
    return " ".join(_clean(frame.get(key)) for key in ("heading", "text", "punch") if _clean(frame.get(key)))


def _subject_terms(story: str) -> set[str]:
    terms = []
    for token in _tokens(story):
        if token in {_stem_token(word) for word in _SUBJECT_STOPWORDS}:
            continue
        if token.startswith("تطور") or token.startswith("حكاي"):
            continue
        terms.append(token)
    if terms:
        return set(terms)
    return set(_tokens(story))


def _copy_subject_overlap(frame: dict[str, Any], subject_terms: set[str]) -> set[str]:
    return set(_tokens(_copy_blob(frame))) & subject_terms


def _visual_subject_overlap(frame: dict[str, Any], subject_terms: set[str]) -> set[str]:
    parts: list[str] = []
    for key in ("image_keywords", "image_keywords_ar"):
        value = frame.get(key) or []
        if isinstance(value, (list, tuple)):
            ...
            parts.append(_clean(value))
    return set(_tokens(" ".join(parts))) & subject_terms


def _focus_label(frame: dict[str, Any], subject_terms: set[str]) -> tuple[str, list[str]]:
    copy_overlap = _copy_subject_overlap(frame, subject_terms)
    visual_overlap = _visual_subject_overlap(frame, subject_terms)
    threshold = 1 if len(subject_terms) <= 2 else 2
    if len(copy_overlap) >= threshold:
        return "DIRECT", sorted(copy_overlap)
    blob = _copy_blob(frame).casefold()
    if any(cue.casefold() in blob for cue in _SUPPORT_CUES):
        return "SUPPORTING", sorted(copy_overlap | visual_overlap)
    if copy_overlap or visual_overlap:
        return "SUPPORTING", sorted(copy_overlap | visual_overlap)
    return "DRIFT", []
...
def _has_any(value: object, cues: Iterable[str]) -> bool:
    folded = _normalize_arabic(_clean(value).casefold())
    return any(_normalize_arabic(cue.casefold()) in folded for cue in cues)
...
def _is_historical_copy(frame: dict[str, Any]) -> bool:
    ...
    if years and max(years) < 2015:
        return True
    return _has_any(_copy_blob(frame), _HISTORICAL_COPY_CUES)


def _saudi_context(story: str, frames: list[dict[str, Any]]) -> bool:
    blob = _normalize_arabic(" ".join([story] + [_copy_blob(frame) for frame in frames]).casefold())
    return any(_normalize_arabic(term.casefold()) in blob for term in _SAUDI_CONTEXT_TERMS)


def _finding(frame: int, dimension: str, code: str, message: str, severity: str = "BLOCK") -> dict[str, Any]:
    return {
        "frame": int(frame),
        "dimension": dimension,
        "severity": severity,
...
def _visual_blob(frame: dict[str, Any], visual_row: dict[str, Any]) -> str:
    ...
    for key in ("image_keywords", "image_keywords_ar"):
        value = frame.get(key) or []
        if isinstance(value, (list, tuple)):
            pieces.extend(_clean(item) for item in value)
        else:
            pieces.append(_clean(value))
    pieces.extend(_string_values(visual_row or {}))
...
def _visual_era(frame: dict[str, Any], visual_row: dict[str, Any]) -> dict[str, Any]:
    ...
    if archival and not (modern and years and max(years) >= 2015):
        ...
        label = "UNKNOWN"
    return {"label": label, "years": years, "evidence": blob}
...
def evaluate_story_quality(
    ...
    dimensions = {
        ...
        "narrative_chronology": "PASS",
        "visual_chronology": "PASS",
        "final_frame_currency": "PASS",
    }
    ...
    for index, frame in enumerate(frames, start=1):
        ...
        visual_row = visual_rows.get(str(index)) or {}
        visual = _visual_era(frame, visual_row)
        ...
        if focus == "DRIFT" or (index in {1, len(frames)} and focus != "DIRECT"):
            ...
            code = "CRITICAL_FRAME_NOT_DIRECT" if index in {1, len(frames)} else "SUBJECT_DRIFT"
            findings.append(_finding(
                index, "subject_focus", code,
        ...
        if any(pattern.search(normalized_copy) for pattern in _EXCLUSIVITY_PATTERNS):
            ...
            findings.append(_finding(
                index, "claim_precision", "UNSUPPORTED_EXCLUSIVITY",
                "exclusive wording requires stronger evidence or narrower phrasing",
            ))
        if any(pattern.search(normalized_copy) for pattern in _UNIVERSALITY_PATTERNS):
            dimensions["claim_precision"] = "BLOCKED"
            findings.append(_finding(
                index, "claim_precision", "UNSUPPORTED_UNIVERSALITY",
                "universal wording is too broad for a release-ready Story",
            ))
        if any(pattern.search(normalized_copy) for pattern in _SUPERLATIVE_PATTERNS):
            dimensions["claim_precision"] = "BLOCKED"
            findings.append(_finding(
                index, "claim_precision", "UNSUPPORTED_SUPERLATIVE",
                "superlative wording needs explicit evidence before release",
            ))
        if saudi_context and "الجزيره" in normalized_copy and "شبه الجزيره" not in normalized_copy:
            dimensions["claim_precision"] = "BLOCKED"
            findings.append(_finding(
                index, "claim_precision", "SAUDI_GEOGRAPHY_AMBIGUOUS",
                "use السعودية or المملكة when the intended referent is the modern Kingdom",
            ))
        ...
        if copy_years:
            ...
            if last_copy_year is not None and first_year < last_copy_year and not flashback:
                ...
                findings.append(_finding(
                    ...
                    f"narrative year regresses from {last_copy_year} to {first_year} without a flashback cue",
                ))
            if not flashback:
        ...
        if current_seen and _is_historical_copy(frame) and not flashback:
            ...
            if not any(f["frame"] == index and f["code"] == "UNEXPLAINED_TIME_REGRESSION" for f in findings):
                findings.append(_finding(
                    index, "narrative_chronology", "CURRENT_TO_HISTORICAL_REGRESSION",
        ...
        if visual_years:
            ...
            if last_visual_year is not None and visual_year < last_visual_year and not flashback:
                ...
                findings.append(_finding(
                    index, "visual_chronology", "VISUAL_TIME_REGRESSION",
        ...
        visual_rank = {"HISTORICAL": 0, "UNKNOWN": 1, "CURRENT": 2}.get(visual["label"], 1)
        if last_visual_rank is not None and visual_rank < last_visual_rank and not flashback:
            dimensions["visual_chronology"] = "BLOCKED"
            findings.append(_finding(
                index, "visual_chronology", "VISUAL_ERA_REGRESSION",
                "visual sequence moves backward in time without narrative reason",
            ))
        if visual["label"] != "UNKNOWN" and not flashback:
            last_visual_rank = visual_rank

        if current and visual["label"] == "HISTORICAL":
            dimensions["visual_chronology"] = "BLOCKED"
            findings.append(_finding(
                index, "visual_chronology", "CURRENT_COPY_ARCHIVAL_VISUAL",
                "present-day copy is paired with clearly archival visual evidence",
            ))
        ...
        if index == len(frames) and current:
            if visual["label"] == "HISTORICAL":
                ...
                findings.append(_finding(
                    index, "final_frame_currency", "CURRENT_COPY_ARCHIVAL_VISUAL",
                    "current ending cannot use an archival visual",
                ))
            elif visual["label"] != "CURRENT":
                dimensions["final_frame_currency"] = "BLOCKED"
                findings.append(_finding(
                    index, "final_frame_currency", "CURRENT_COPY_UNPROVEN_VISUAL",
                    "current ending needs positive modern/current visual evidence",
                ))

    status = "BLOCKED" if any(item.get("severity") == "BLOCK" for item in findings) else "PASS"
    report = {
        "policy": QUALITY_POLICY,
        "status": status,
        "story": story,
        "story_subject_terms": sorted(subject_terms),
