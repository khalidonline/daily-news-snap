...
- Next bounded implementation assessment: use the owner's already-authorized image reuse to remove an unnecessary three-distinct-photos dependency where one verified subject photo plus information-led frames can support the story. Inspect current preflight/render assumptions first; preserve at least one verified photo per depicted subject, truthful event/archival use, image review, and sources-card exclusion. Do not substitute unrelated stock people or relax licensing. If feasible, implement/test this as a separate small change before any paid full run.
