...
_FINANCE_POLICY_ONLY_RES = (
    ...
    re.compile(
        r"(?:ساما).{0,20}(?:تبعه|تبع\s+الفيدرالي|يمشي\s+وراء\s+الفيدرالي)",
        re.IGNORECASE,
    ),
    re.compile(
        r"(?:وساما\s+تبعه|ساما\s+تبعه)",
        re.IGNORECASE,
    ),
)
...
def enhance_prompt(base_prompt: str, today: date | None = None) -> str:
    ...
    prompt += (
        ...
        "السعر بحسب موعد إعادة التسعير المحدد في العقد. تحركات ساما تؤثر في البيئة النقدية "
        "لكنها ليست وحدها ما يحدد كل قسط متغير.\n"
        "- لا تقل «وساما تبعه» أو أي صياغة توحي بأن ساما ينسخ قرار الفيدرالي آلياً. إذا "
        "أبقى ساما سعر الريبو دون تغيير، قل ذلك حرفياً فقط عندما يؤكده مصدر رسمي.\n"
        ...
        "الفعلية تعتمد على المؤشر المرجعي في العقد ونوع المنتج وهامش البنك وموعد إعادة "
        "التسعير. لا تقل إن الفيدرالي «يحدد قسطك» أو إن قراره «يوصلك خلال أيام»، ولا تقل "
        "إن التمويل المتغير يتحرك فقط عندما يغيّر ساما الريبو.\n"
